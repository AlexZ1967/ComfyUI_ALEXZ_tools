...
def module_worktree_signature(
    ...
    *,
    ...
    if is_git != "true":
        return ""
    # Ignore untracked files to avoid false-positive local-change markers from
    # runtime artifacts/caches generated by other custom nodes.
    status = run_git(
        ["git", "-C", str(module_dir), "status", "--porcelain", "--untracked-files=no"],
        2.0,
    )
    if not status:
        return ""
