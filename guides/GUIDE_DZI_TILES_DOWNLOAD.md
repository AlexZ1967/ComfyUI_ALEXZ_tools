...
- При ошибке первого тайла консоль показывает краткую причину и URL. Полная матрица
  transport/proxy попыток и traceback записываются в
  `logs/image_download_dzi_tiles.log` внутри папки ноды; пароли из proxy URL в
  этом логе маскируются.
