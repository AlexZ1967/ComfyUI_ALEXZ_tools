...
## Назначение
Подгонка цвета `image` к `reference` по методам (`mean_std`, `linear`, `tone_curve`, `adain`, `lab_cdf`, `oklab_cdf`, `perceptual_vgg_fast`).
...
| `preset` | Метод цветокоррекции | `mean_std` — быстро, `linear` — дефолт, `tone_curve` — подгонка контраста, `adain` — быстро и перцептивно, `lab_cdf` — Lab, `oklab_cdf` — Oklab (лучше), `perceptual_vgg_fast` — VGG |
...
| `adain` | Adaptive Instance Normalization | ⚡⚡ | ⭐⭐⭐ | Перцептивная нормализация (новый) |
...
**tone_curve:**
- Подгоняет люминансность через кривую тонов
- Вычисляет квантили (черный, темные, средний, светлые, белый)
- Лучше всего работает для видео с разными экспозициями
- Сохраняет детали в тенях и светах

**adain:**
- Adaptive Instance Normalization из neural style transfer
- Нормализует каждый канал отдельно, затем масштабирует по эталону
- Быстро и стабильно, хороший баланс
- Хорошо для быстрой цветокоррекции
...
- Применяет интерполяцию для создания гладкой кривой отображения
...
- Менее чувствителен к экстремальным значениям (пиксели на границах)
...
- Если видна цветовая нестабильность → `tone_curve` (для экспозиции) или `oklab_cdf` (для глубокой коррекции)
