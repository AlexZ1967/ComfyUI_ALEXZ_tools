...
from .utils.gpu_utils import gpu_memory_manager, cleanup_gpu
...
def compute_flow(
    raft_model: RAFT_bi, frames: torch.Tensor, config: ProPainterConfig, skip_empty_cache: bool = False
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute forward and backward optical flows using the RAFT model.
    
    Args:
        raft_model: The RAFT model instance
        frames: Input frames tensor
        config: Configuration object
        skip_empty_cache: If True, skip empty_cache() calls for better throughput.
                         Use False (default) if OOM is a concern.
    """
    max_hw = max(frames.size(dim=-2), frames.size(dim=-1))
    if max_hw <= 640:
        short_clip_len = 12
    elif max_hw <= 720:
        short_clip_len = 8
    ...
    if frames.size(dim=1) > short_clip_len:
        ...
        for chunck in range(0, config.video_length, short_clip_len):
            ...
            # Respect skip_empty_cache parameter for throughput optimization
            cleanup_gpu(force=not skip_empty_cache)

        gt_flows_f = torch.cat(gt_flows_f_list, dim=1)
        gt_flows_b = torch.cat(gt_flows_b_list, dim=1)
        gt_flows_bi = (gt_flows_f, gt_flows_b)
    else:
        with gpu_memory_manager(empty_cache=not skip_empty_cache):
...
def image_propagation(
    ...
    )  # cap at 100 frames for image propagation
    if config.video_length > subvideo_length_img_prop:
        ...
        for f in range(0, config.video_length, subvideo_length_img_prop):
            ...
            with gpu_memory_manager(empty_cache=not config.skip_empty_cache):
...
def process_inpainting(
    ...
    with torch.inference_mode():
        gt_flows_bi = compute_flow(
            models.raft_model, frames, config, skip_empty_cache=config.skip_empty_cache
        )
        ...
        pred_flows_bi = complete_flow(
            ...
            gt_flows_bi,
            flow_masks,
            config.subvideo_length,
            config.skip_empty_cache,
        )
