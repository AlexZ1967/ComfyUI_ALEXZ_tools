...
_SMOOTHNESS_KERNEL_CACHE = {}
_SECOND_ORDER_KERNEL_CACHE = {}


def _get_smoothness_kernels(device, dtype):
    key = (device, dtype)
    weights = _SMOOTHNESS_KERNEL_CACHE.get(key)
    if weights is None:
        ...
        _SMOOTHNESS_KERNEL_CACHE[key] = weights
    return weights


def _get_second_order_kernels(device, dtype):
    key = (device, dtype)
    weights = _SECOND_ORDER_KERNEL_CACHE.get(key)
    if weights is None:
        weights = torch.tensor(
            [
                [[[0, 0, 0], [1, -2, 1], [0, 0, 0]]],
        ...
        _SECOND_ORDER_KERNEL_CACHE[key] = weights
    return weights
...
def initialize_RAFT(model_path="weights/raft-things.pth", device="cuda"):
    ...
    model = RAFT(args)
    state_dict = torch.load(args.raft_model, map_location="cpu")
...
def smoothness_deltas(flow):
    ...
    weights = _get_smoothness_kernels(flow.device, flow.dtype)
...
def charbonnier_loss(x, mask=None, truncate=None, alpha=0.45, beta=1.0, epsilon=0.001):
    ...
    b, c, h, w = x.shape
    norm = b * c * h * w
    epsilon = x.new_tensor(epsilon)
    error = (x * beta).pow(2).add(epsilon * epsilon).pow(alpha)
    if mask is not None:
        error = mask * error
...
def second_order_deltas(flow):
    ...
    weights = _get_second_order_kernels(flow.device, flow.dtype)
...
def create_mask(tensor, paddings):
    ...
    inner = tensor.new_ones((inner_height, inner_width))
    torch_paddings = [
        paddings[1][0],
        paddings[1][1],
        paddings[0][0],
