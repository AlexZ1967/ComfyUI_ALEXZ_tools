...
Ключевые входы: `reference`, `image`, `strength`, `compute_device` (`auto`/`cpu`/`cuda`), `color_space` (`rgb`/`oklab`), `downscale_long_side` (`as_is`/`1080p`/`720p`/`480p`), `seam_model` (`v2_tonal`/`v3_hybrid`/`v1_affine`), `steps`, `lr`, `w_mse`, `w_ssim`, `w_grad`, `reg_weight`, `robust_delta`, `hybrid_residual_strength`, `hybrid_residual_reg`, `hybrid_coherence_reg`.  
