...
- По кнопке `Обновить` выполняется `git fetch --quiet` для custom-модулей с настроенным upstream, чтобы статус `🟥` (`behind > 0`) отражал актуальное состояние GitHub.
- В консоль ComfyUI добавлено пошаговое логирование процесса refresh: старт, прогресс по модулям при sync, пересчет snapshot'ов, завершение.
